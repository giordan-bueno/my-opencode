...
- **Project name**: $1
- **Watermarked PDF files**: $2 .. $N (all arguments after the project name)
...
- The subagent should also create a `.gitignore` file inside the `$1/` folder using the standard project template defined in `@pdf-cleaner` (`.opencode/agents/pdf-cleaner.md`).
...
- Use commit message: `docs($1): add project rules, requirements, subtask template, and subagents from instruction PDFs`